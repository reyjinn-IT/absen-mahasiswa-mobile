...
import { dosenService, type DosenKelas } from '@/services/dosen';

export default function DosenKelasScreen() {
  const [data, setData] = useState<DosenKelas[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);

  const fetchData = useCallback(async () => {
    try {
      const res = await dosenService.getKelasDosen();
      setData(res);
    } finally {
      setLoading(false);
      setRefreshing(false);
      ...
      <SafeAreaView style={styles.safeArea}>
        <ScrollView
          ...
          {loading ? (
            ...
            <EmptyState title="Belum ada kelas" message="Anda belum mengajar kelas apapun" />
          ) : (
                ...
                <Card key={k.id} style={styles.card}>
                  <ThemedView style={styles.cardContent}>
                    ...
                    <Button
                      ...
                      size="sm"
                      variant="secondary"
                      onPress={() => router.push(`/absensi?kelasId=${k.id}`)}
                    />
                  </ThemedView>
                </Card>
              ))}
